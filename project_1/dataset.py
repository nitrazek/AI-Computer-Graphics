...
class ImageDataset(Dataset):
    def __init__(self, input_paths, target_paths):
        self.input_paths = sorted([os.path.join(input_paths, f) for f in os.listdir(input_paths) if f.endswith('.png')])
        self.target_paths = sorted([os.path.join(target_paths, f) for f in os.listdir(target_paths) if f.endswith('.png')])
